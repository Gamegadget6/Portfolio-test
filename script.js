...
(function () {
        ...
        for (let i = comets.length - 1; i >= 0; i--) {
            ...
            if (c.trail.length > 1) {
                ...
                const grad = ctx.createLinearGradient(
                    c.trail[0].x, c.trail[0].y, c.x, c.y
                );
    ...
    function tick(now) {
        ...
        for (let i = 0; i < stars.length; i++) {
            ...
            const alpha = Math.min(s.baseAlpha * (0.55 + 0.45 * twinkle) + s.wake * 0.55, 1);
            const size = s.r * (1 + s.wake * 1.6);

            if (s.wake > 0.08) {
                const glowSize = size * 10;
                ctx.globalAlpha = Math.min(s.wake * 0.8, 0.9);
                ctx.drawImage(glowGold, s.x - glowSize / 2, s.y - glowSize / 2, glowSize, glowSize);
            }
...
(function () {
        ...
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
                ...
                railDots.forEach((dot) => {
                    dot.classList.toggle('active', dot.dataset.rail === id);
                });
            }
        });
    ...
    function setupReveals() {
        [...document.querySelectorAll('.section-header'), ...document.querySelectorAll('.contact-plate')]
            .forEach((el) => el.classList.add('reveal'));

        [...document.querySelectorAll('.skills-grid'), ...document.querySelectorAll('.projects-grid')]
